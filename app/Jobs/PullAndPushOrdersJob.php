...
use App\Services\Orders\ShopifyOrderCreator;
...
class PullAndPushOrdersJob implements ShouldQueue
{
    ...
    {
        $channel = Channel::with('shop')->findOrFail($this->channelId);
        ...
        foreach ($registry->for($channel)->fetchOrders($since) as $ord) {
            ...
            if (!$co->imported_to_shopify) {
                $sid = ShopifyOrderCreator::push($co);
                if ($sid) $co->update(['imported_to_shopify' => true, 'shopify_order_id' => $sid]);
            }
        }
    }
}
